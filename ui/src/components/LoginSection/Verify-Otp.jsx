    ...
    const onSubmit = async(data) => {
          ...
          try {
             ...
             if(result.status === 'success'){
                toast.success(result.message || 'Account verified successfully! Redirect to home page.')
                navigate('/')
             }
             else{ 
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-800">
      <div className="bg-gray-900 text-white p-8 rounded-lg shadow-lg max-w-md w-full">
        ...
        <div className="text-sm text-gray-400 text-center mt-4">
            Didn't receive code? <a className="font-medium text-indigo-400 hover:text-indigo-300 ">Resend</a>
        </div>
      </div>
    </div>
  );
